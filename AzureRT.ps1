...
Function ImportAll {
    import-module Az
    write-host "[+] Imported Az..."
    import-module AzureAD
    write-host "[+] Imported AzureAD..."
    #import-module C:\AzAD\Tools\MSOnline\MSOnline.psd1
    #write-host "[+] Imported MSOnline..."
    #import-module C:\AzAD\Tools\AADInternals\AADInternals.psd1
    #write-host "[+] Imported AADInternals..."
    write-host "[+] Done! Imported all modules."
}
Function List-DeviceOwners {
    $users = (Get-AzureADUser | select -ExpandProperty UserPrincipalName)
    foreach ($user in $users) {
        if (Get-AzureADUserOwnedDevice -ObjectId $user) {
            Write-Host -ForegroundColor Green "*** Devices owned by $user :"
            Get-AzureADUserOwnedDevice -ObjectId $user
...
Function Evil-Winrm {
    ...
    $loginName = $TargetHostName + "\" + $UserName
    $SecPassword = ConvertTo-SecureString $Password -AsPlainText -Force
    $cred = New-Object System.Management.Automation.PSCredential($loginName, $SecPassword)
    $script:evilSession = New-PSSession -ComputerName $TargetIP -Credential $cred
...
Function Connect-All {
    ...
    if ($cred) { Write-Host "Connecting..." }
    Connect-ARTAD -Credential $cred
    Connect-ART -Credential $cred
...
Function whois {
    ...
    $objID = (Get-AzureADUser -ObjectId $Target).ObjectId
    $upn = (Get-AzureADUser -ObjectId $Target).UserPrincipalName
    Write-Host ""
    Write-Host -ForegroundColor Green "[+] Target set to: $upn - $objID"
    Write-Host ""
    Write-Host -ForegroundColor Green  "[+] Group Memeberships:"
    Get-AzureADUserMembership -ObjectId $objID | fl displayname,Description,ObjectId
    Write-Host ""
    Write-Host "_________________________________________________"
    Write-Host ""
    Write-Host -ForegroundColor Green  "[+] Role Assignments:"
    Get-AzRoleAssignment -ObjectId $objID | fl RoleDefinitionName,scope
    Write-Host ""
    Write-Host "_________________________________________________"
    Write-Host ""
    Write-Host -ForegroundColor Green  "[+] Administrative Unit Memeberships:"
    Write-Host ""
    $administrativeUnits = Get-MsolAdministrativeUnit
    foreach ($administrativeUnit in $administrativeUnits) {
        $administrativeUnitName = $administrativeUnit.DisplayName
        
        
        $isMember = Get-MsolAdministrativeUnitMember -AdministrativeUnitObjectId $administrativeUnit.ObjectId -All | 
                    Where-Object {$_.EmailAddress -eq $upn}
    
        if ($isMember) {
            $adminUnitDesc = (Get-MsolAdministrativeUnit -ObjectId $administrativeUnit.ObjectId).Description
            Write-Host "$upn is a member of Administrative Unit: $administrativeUnitName"
            Write-Host "Administrative Unit Description: $adminUnitDesc"
            $scopedRole = Get-AzureADMSScopedRoleMembership -Id $administrativeUnit.ObjectId -ErrorAction SilentlyContinue
            if ($scopedRole) {
                $actualrolID = $scopedRole.RoleId
                $actualroleName = (Get-AzureADDirectoryRole -ObjectId $actualrolID).DisplayName
                $actualrolDesc = (Get-AzureADDirectoryRole -ObjectId $actualrolID).Description
                $actualrolAssignee = (Get-AzureADMSScopedRoleMembership -Id $administrativeUnit.ObjectId -ErrorAction SilentlyContinue | select -ExpandProperty rolememberinfo).UserPrincipalName
                Write-Host -ForegroundColor Red "[+] Attention: Admin Unit has Scoped Role(s)!"
                Write-Host "[+] Scoped Role: $actualroleName"
                Write-Host "[+] Scoped Role Description: $actualrolDesc"
                Write-Host "[+] Scoped Role Assignee: $actualrolAssignee"

            }
        } 
    #else { Write-Host "$upn is NOT a member of $administrativeUnitName" }
    }
    Write-Host ""
}
######################################
